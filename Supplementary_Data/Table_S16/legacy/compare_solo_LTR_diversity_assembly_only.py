...
SEQ=ROOT.parent/'HML2_project_data/processed_loci'
groups=defaultdict(list)
source=[]
seen=set()
for r in csv.DictReader(CAT.open(),delimiter='\t'):
    if r['analysis_include']!='1' or not re.fullmatch(r'(HG|NA)\d+',r['ID']) or r['observation_state']!='PRESENT' or r['Structure']!='Solo-LTR':
        continue
    key=(r['Locus'],r['ID_Full'])
    if key in seen: continue
    seen.add(key)
    p=SEQ/r['Locus']/(r['ID_Full']+'.fa')
    if p.is_file():
        seq=''.join(x.strip() for x in p.read_text().splitlines() if not x.startswith('>')).upper()
        groups[r['Locus']].append(seq)
        source.append({'locus':r['Locus'],'ID_Full':r['ID_Full'],'sequence_path':str(p)})
rows=[]; skipped=[]
...
reference=ROOT/'project/inputs/references/ltr_detection_ref.fa'
completed=subprocess.run(['historical_source/opt/anaconda3/bin/minimap2','-x','asm20','-c','--secondary=no',str(reference),str(P/'solo_comparison_unique.fa')],capture_output=True,text=True,check=True)
(P/'solo_comparison_LTR_mapping.paf').write_text(completed.stdout)
projected={}; mapping=[]
...
mu=1.2e-9; L=968
expectations=[{'time_years':t,'pairwise_rate_per_site_per_My':r,'expected_differences_968bp':r*t/1e6*L,'probability_zero':math.exp(-r*t/1e6*L)} for t in range(0,1000001,10000) for r in [.0024,.0045]]
summary={'source_catalog':str(CAT),'sequence_root':str(SEQ),'LTR_reference':str(reference),'method':'Same retained sequence availability and n>=20 rule as current Figure 7 builder. Reference alignment also permits variable-length extractions. Unique sequences mapped by minimap2 asm20 to the 968-bp LTR detection reference to remove host flanks. ACGT pairs only at aligned reference positions. Insertions and gaps are excluded from the substitution-distance measure.',
    ...
    'sequence_divergence_expectations':expectations,
