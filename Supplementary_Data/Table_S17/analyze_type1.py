...
SRC = ROOT/'hervk-hml2-age/manuscript_figures/python/analysis/type1_cassette_mechanism'
AUTH = ROOT/'project/working/type1_ltr_authority_resolution_agent/subfamily_authority.tsv'
PRIM = ROOT/'project/working/ape_recombination_origin_v1/inputs/ape_candidates_kcon_projection.fa'
LEDGER = ROOT/'project/working/ape_genomewide_hml2_scan_v1/results/candidate_ledger.tsv'
CAN = set('ACGT')
def rows(p):
...
manifest=rows(SRC/'derived/representative_manifest.tsv')
authority={r['locus']:r for r in rows(AUTH)}
aln=fasta(SRC/'derived/representatives.mafft.fasta.gz')
...
windows=json.loads((SRC/'results/analysis_parameters.json').read_text())['windows']
...
old=rows(SRC/'results/pairwise_divergence_summary.tsv')
for r in old:
...
provenance=[SRC/'derived/representatives.mafft.fasta.gz',SRC/'derived/representative_manifest.tsv',SRC/'results/analysis_parameters.json',SRC/'results/pairwise_divergence_summary.tsv',AUTH,PRIM,LEDGER]
